...
namespace MobleFinal
{
    public partial class UserInfoForm : Form
    {
        ...
        private void Update_listview()
        {
            ...
            item.Text = "1";
            item.SubItems.Add(user.Name); // 두 번째 열에 이름 추가
        ...
        private void ShowUserInfo(User user)
        {
            ...
            if (user.Authority == 1)
            {
                User_Authoritys.Text = "일반 사용자";
            }
            else if (user.Authority == 2)
            {
                User_Authoritys.Text = "관리자";
            }
        }
        ...
        private void bt_upload_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("비밀번호를 입력하세요.", "경고", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return; // 이후 코드 실행 중지
            }
            ...
            // 사용자가 선택한 권한 확인
            int authority = 0;
            if (User_Authoritys.Text != null)
            {
                if (User_Authoritys.Text.ToString() == "일반 사용자")
                {
                    authority = 1;
                }
                else if (User_Authoritys.Text.ToString() == "관리자")
                {
                    authority = 2;
                }
            }
            ...
            if (User_list.SelectedItems.Count > 0)
            {
                ...
                selectedUser.Password = User_password.Text;
